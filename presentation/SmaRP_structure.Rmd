...
* The files `NAMESPACE` & `DOCUMENTATION` indicate the package dependencies.
* The `README.md` file provides information about the package


## SmaRP Structure Advantages

This configuration has multiple advantages:
...
* Easy documentation of version changes via `NEWS.md` an release tags
* Clear identification of dependencies (package won't build if requirements are not met)

## SmaRP Structure Disadvantages

To be consistent with such a structure, it is important to:

- on one hand keep all data manipulations as functions under the `R` folder
- on the other hand to embed all app related functionalities under `ui.R`, `server.R` and `global.R`
