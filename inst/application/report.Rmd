...
```{r, echo = FALSE}
Gemeinde <- chartr("äöü", "aeo", params$gemeinden)

```

* Residence in `r Gemeinde`, `r params$Kanton`, postal code `r params$postalcode`.
...
The municipality-dependent multiplication factor for municipality  `r Gemeinde`  is  `r fgemeinde`.

The church affiliation dependent multiplication factor for municipality `r Gemeinde` is `r fkirche`. If there is no church affiliation this factor is  `0`.

[^4]: AHV salary: 2350 month, 28200 year.
[^5]: 831.40. Bundesgesetz Ã¼ber die berufliche Alters-, Hinterlassenen- und Invalidenvorsorge (Art.16)
