...
barGraphData.columns <- grep("annotation$", barGraphData.columns, value = TRUE, invert = TRUE)
barGraphData.long <- melt(params$BarGraphData,
                          id = "contribution",
                          measure = barGraphData.columns)
barGraphData.long$variable <- factor(barGraphData.long$variable, levels = sort(barGraphData.long$variable, decreasing = TRUE))

ggplot(data = barGraphData.long, aes(x = contribution, y = value, fill = variable))+
