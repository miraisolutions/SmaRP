...
If married double-income, SmaRP assumes that all the inputs correspond to the aggregated amounts and a 50% income distribution is assumed. 
...
* The contribution to the Pillar II is of `r params$P2purchase` monetary units, and is purchased as a `r TypePurchase`.

T* he interest rate of of the Pillar II is given by `r params$rate`, and we ensure that the amount considered in the calculation is always higher than the minimum required by law.
...
```{r taxrate, echo=FALSE}
if(is.null(params$TaxRate)){
  taxrate <- "not provided as an input and it is therefore calculated"
} else {
...
* The postal code of residence `r params$postalcode` is used to infer the kanton of residence `r params$Kanton` and can be used for the tax rate computation.

* The user's marital status is `r params$rate_group`, with  `r params$NKids`  children. This information has an inpact on the total tax benefits.
...
$$ PensionableSalary(t) = max(Salary(t) - \frac{7}{8} AHV Salary(t), 30 * AHVSalary) $$ [^1]
...
$$ TaxBenefit(t) = TaxPaid_{Salary}(t) - TaxPaid_{TaxableIncome}(t) $$[^4]
